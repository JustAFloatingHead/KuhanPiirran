...
def command_lists(list_name:str):
    ...
    if list_name=="draw6":
        lista=["LOOP(VCT)","IF(TXT,TXT,TXT,OPT)","empty","more..."]
    ...
    if list_name=="edit":
        lista=["abs stretch(SP,OPT)|","add text","add function","change(FILE)|","copy|","deletefunction(TXT)|","depth(NRO,OPT)|"]
        lista+=[ "destroy|","glue file|","merge(TXT)|","put(FILE)|","rel stretch(SP,OPT)","rotate(ANG,OPT)|","scale(SCL,OPT)|"]
...
def options_popup():
    ...
    recycling_button=tk.Button(popup_canvas,text="Recycling: on",command=recycling_switch,bg="lightblue",width=9)
    ...
    def ready(filename,actiontype):
        ...
        if pd["style"] in ["rectangle","triangle"]:
            ...
            if pd["style"]=="rectangle": #we don't need back wtih triangles
                back_save_name=PngMaker.from_png_to_Drawing(filename,back_prop,savename="drawings\\back"+save_name+".txt")#this both saves the file
                back_save_name= filename_without_subdir(back_save_name,"drawings") 
        if pd["style"] in ["polygon"]:
            save_name=PngMaker.png_to_polygon_Drawing(filename,prop)#this both saves the file and
            save_name= filename_without_subdir(save_name,"drawings") 
            #returns the name of the file and also creates it in this same method
            back_save_name=PngMaker.png_to_polygon_Drawing(filename,back_prop,savename="drawings\\back"+save_name+".txt")#this both saves the file and
            back_save_name= filename_without_subdir(back_save_name,"drawings") 
        if pd["style"]=="simple":
            save_name=PngMaker.from_photo_to_cartoon(filename,prop) 
            save_name= filename_without_subdir(save_name,"drawings") 
            back_save_name=PngMaker.from_photo_to_cartoon(filename,back_prop,savename="drawings\\back"+save_name+".txt") 
            back_save_name= filename_without_subdir(back_save_name,"drawings") 
        ...
        if actiontype==1 and pd["style"] in ["polygon","rectangle"]:
            ...
            back_filetext= layer_items.load_file_as_string(back_save_name,"drawings")#we now put this on different layer, 
...
def matrix_str_to_actual_matrix(matrix_str):
    vector_strs=FunctionOperator.vector_components(matrix_str)
    matrix=[]
    for vector_str in vector_strs:
        vector=FunctionOperator.vector_components(vector_str)
...
def draw_coordinates(origo_value=(ORIGOX,ORIGOY),xscale=SCALEX,yscale=SCALEY,
    ...
    #KESKEN, origon sijainti täytyy huomioida
    ...
    if mode=="drawing_mode": 
        ...
        #for com in array: #tässä outo bugi, oli japteltu kahteen osaan toteutus
            if com[0:5]=="dots(":
                ...
                matrix_drawing_orders(matrix_str_to_actual_matrix(matrix_str),x_axis_index,y_axis_index)
                draw_temporary_file()
                set_turtle_values(temp_turtle,ttvalues[0],ttvalues[1],ttvalues[2],ttvalues[3],ttvalues[4])
                basic_info()
...
def timed_execution():
    ...
    quick_png((-(int(QUICK_WIDTH/2)),int(QUICK_HEIGHT/2)),(int(QUICK_WIDTH/2),-(int(QUICK_HEIGHT/2)))) #added 30.11, this creates pngs in purpose to make animation 
